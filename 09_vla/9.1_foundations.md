...
> Vision-Language-Action 모델의 기반이 되는 개념들. Robot manipulation, embodied AI의 기초.
...
전문가(expert)의 행동 데모를 **지도학습으로 직접 모방**함. 가장 단순한 로봇 학습 방법.
...
**단점 - Covariate Shift**:
테스트 시 모델이 작은 오류를 범하면, 이후 상황이 학습 분포를 벗어남.
오류가 누적되어 큰 실패로 이어짐(compounding error).
...
환경과 직접 상호작용하지 않고, **미리 수집된 데이터셋으로만** 학습함.
...
데이터셋에 없는 (s, a) 쌍에 대한 Q-value 추정이 부정확함.
→ Conservative Q-Learning (CQL), Implicit Q-Learning (IQL) 등으로 해결함.

### Imitation Learning vs Behavior Cloning

Imitation Learning은 BC보다 넓은 개념:
- **BC**: 단순 지도학습으로 모방
...
LLM처럼 action을 **token by token으로 autoregressive하게 생성**함.
...
**Action tokenization**: 연속적인 action (6-DOF joint angles 등)을 discrete bin으로 양자화함:
...
VLA는 이 두 가지를 하나의 모델로 처리하려 함. Language와 vision의 고수준 이해력을 갖추면서 동시에 정밀한 motor control도 해야 함.
...
**World model**은 환경의 다음 상태를 예측하는 모델:
...
**최근 동향**: Large-scale video pretraining으로 학습된 world model이 robotics에 적용되고 있음 (UniSim, GR-1 등).
