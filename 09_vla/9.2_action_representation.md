...
> VLA에서 action을 어떻게 표현하고 예측할지에 대한 핵심 설계 선택들.
...
연속적인 action 값을 **유한한 bin으로 양자화**함:
...
Action을 **diffusion model로 예측**함. Random noise에서 시작하여 반복적으로 denoising하여 action sequence를 생성함.
...
**Multimodal action distribution 처리**: 같은 상황에서 여러 equally valid한 action이 있을 때, 단순 regression은 평균을 예측하여 실제로는 invalid한 action을 출력함. Diffusion은 여러 mode를 모두 포착할 수 있음.
...
Diffusion의 대안으로, 더 빠른 생성과 더 단순한 훈련을 제공함.

### 개념

Noise에서 데이터로의 **직선 경로(straight path)**를 학습함:
...
π0 (Physical Intelligence)가 flow matching을 action prediction에 적용함.

---

## Chunked Action Prediction

매 timestep마다 하나의 action을 예측하는 대신, **여러 step의 action을 한 번에 예측**함:
...
실시간 피드백을 반영하면서도 temporal consistency를 유지함.

**Execution horizon T_a**: 실제로 실행하는 step 수. T_a < H이면 더 자주 재계획함.
