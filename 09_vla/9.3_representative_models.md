...
Google Robotics의 첫 번째 대규모 로봇 학습 모델.
...
RT-1에서 한 발 더 나아가, **pretrained VLM을 기반**으로 로봇을 학습함.
LLM/VLM의 웹 지식을 로봇 제어로 전달함.
...
오픈소스 generalist robot policy. 누구나 사용하고 fine-tuning할 수 있도록 설계됨.
...
Vision-Language-Action 아키텍처의 공개 baseline 모델.
...
Physical Intelligence(π0 as company)에서 개발한 generalist robot policy. Diffusion-based action prediction으로 높은 dexterity를 달성함.
...
다양한 로봇 플랫폼(UR5, Franka, Spot, etc.)에서 수집된 데이터를 함께 학습함.
...
**현실적 접근**: Generalist를 pretraining으로 사용하고, task-specific fine-tuning으로 성능을 높임.
