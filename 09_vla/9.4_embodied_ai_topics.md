...
> VLA를 실제 로봇 시스템에 적용할 때 마주치는 issue들.
...
언어 모델의 "tool use"(API 호출, 코드 실행)를 **로봇의 도구 사용**으로 확장함.
...
복잡한 long-horizon task를 **레벨별로 분해**함.
...
**SayCan (Google, 2022)**: LLM의 language probability와 affordance model(현재 환경에서 실행 가능성)을 결합하여 실현 가능한 계획을 선택함:
...
VLA는 주로 task planning을 담당하고, motion planning은 classical robotics 방법(RRT, optimization-based)이 담당함.
...
자연어 명령을 이해하고, 그에 맞는 manipulation task를 수행함.
...
RT-2가 Level 3-4에서도 동작함을 보인 것이 큰 의미가 있었음.
