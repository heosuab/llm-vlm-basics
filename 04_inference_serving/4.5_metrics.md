...
높은 throughput = GPU를 효율적으로 사용한다는 의미.
...
**Input vs Output throughput**: Output token 생성이 더 병목이 됨 (decode phase는 memory-bound).
...
요청이 들어온 시점부터 **첫 번째 output token**이 반환될 때까지의 시간:
...
사용자 경험에서 "응답이 시작되는 느낌"을 결정함. Streaming 응답에서 특히 중요함.
...
긴 prompt를 청크로 나눠서 prefill과 decode를 번갈아 처리함:
→ 긴 prompt가 decode(다른 request)를 블락하는 시간을 줄여 TTFT를 개선함.
...
P95, P99 latency입니다. 요청의 95% 또는 99%가 이 시간 내에 완료됨.
