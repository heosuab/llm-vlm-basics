...
> Autoregressive generation에서 다음 token을 어떻게 선택할지 결정하는 전략들.
...
상위 k개의 가설(hypothesis)을 동시에 유지하면서 탐색함:
...
길이 페널티 없으면 짧은 시퀀스가 유리함 (log probability가 음수이므로 길수록 낮아짐).
...
**문제**: 확률 분포가 뾰족하거나 넓은 경우를 구분하지 못함.
...
누적 확률이 p를 넘을 때까지의 token들로만 샘플링함:
...
→ 상황에 맞게 자동으로 후보 수가 조정됨.

**p=0.9**: 상위 90% 확률 질량에 해당하는 token들만 사용
**p=0.95**: 더 많은 후보, 더 다양한 출력

Top-p는 top-k보다 더 robust한 방법으로, 현재 가장 널리 사용됨.
...
이미 생성한 token들의 logit에 패널티를 적용함:
...
일반적으로 α = 0.6~1.0을 사용함.

---

## Constrained Decoding

출력이 특정 제약 조건을 만족하도록 강제함.

### Trie 기반 제약

특정 단어나 구문이 **반드시 포함**되어야 할 때 사용함:
...
출력이 반드시 valid JSON이 되도록, 그리고 특정 schema를 따르도록 강제함.

### Character-Level FSM (Finite State Machine)

JSON의 문법을 FSM으로 표현하고, 각 state에서 valid한 다음 token만 허용함:
...
각 생성 step에서 현재 JSON state에서 **invalid한 token들의 logit을 -inf**로 설정함:
...
Outlines, Guidance, SGLang 라이브러리가 이 기능을 제공함.
