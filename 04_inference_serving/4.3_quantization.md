...
> 모델의 파라미터를 낮은 precision으로 표현하여 메모리와 추론 속도를 개선.

---

## 양자화(Quantization)란?

FP32/BF16의 고정밀도 값을 INT8/INT4/FP8 등 낮은 정밀도로 근사함:
...
**학습 후**에 이미 완성된 모델을 양자화함. 재학습이 필요 없음.
...
**Weight만** INT4/INT8으로 양자화하고, activation은 FP16으로 유지함.
...
dequantize 오버헤드가 있지만, **메모리 bandwidth가 병목인 decode 단계**에서는 매우 효과적.
...
Weight와 Activation **둘 다** INT8로 양자화함:
...
**문제**: LLM의 activation에는 **outlier**가 있음
...
**주의**: key와 value의 양자화 민감도가 다릅니다. Key는 attention score 계산에 직접 사용되어 더 민감함.
...
모든 weight가 똑같이 중요하지 않음. **활성화 크기가 큰(salient) weight**는 더 정밀하게, 그렇지 않은 weight는 낮은 정밀도로 양자화함.
...
Layer-wise 양자화를 **2차 정보(Hessian)**를 활용하여 수행함.

```
목표: min ||WX - Ŵ X||²_F  (W: 원래 weight, Ŵ: 양자화된 weight)
```

각 weight를 양자화할 때 발생하는 오차를 **나머지 weight 조정으로 보상**함:
...
H100 GPU에서 FP8 tensor core를 네이티브 지원함.
...
**DeepSeek-V3**: FP8 mixed-precision으로 학습하여 비용을 크게 줄였음.
