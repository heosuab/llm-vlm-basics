...
> LLM 추론을 빠르고 효율적으로 만드는 핵심 기법들을 다룹니다.

---

## KV-Cache

자세한 내용은 [1.2 Attention Details](../01_transformer_architecture/1.2_attention_details.md)에서 다뤘습니다. 여기서는 실용적 관점을 추가합니다.
...
긴 시퀀스와 많은 동시 요청이 있으면 KV-cache가 GPU 메모리의 상당 부분을 차지합니다.

---

## Prefill vs Decode Phase

LLM 추론은 두 단계로 나뉘며, 각각 다른 병목이 있습니다.
...
vLLM에서 개발한 혁신적인 KV-cache 메모리 관리 방법입니다.

### 문제: KV-Cache 메모리 낭비

기존 방식에서는 각 request에 **최대 시퀀스 길이만큼** 메모리를 미리 할당합니다:
...
→ 메모리 조각화(fragmentation)와 낭비로 동시 처리 가능한 request 수가 제한됩니다.

### 해결책: OS Virtual Memory 차용

OS의 페이지 테이블 방식을 KV-cache에 적용합니다:
...
**각 iteration(step)마다** 완료된 request를 제거하고 새 request를 추가합니다:
...
**효과**: GPU utilization이 크게 향상됩니다. 처리량이 최대 수십 배 증가하는 경우도 있습니다.
...
작은 draft model로 여러 token을 미리 생성하고, 큰 target model이 한 번의 forward pass로 검증합니다.
...
**보장**: Target model의 분포와 **완전히 동일한** 출력 분포를 유지합니다 (exact decoding).

### 실용적 고려사항

- Draft model과 target model은 **같은 tokenizer**를 사용해야 합니다
...
각 token을 항상 모든 레이어를 거칠 필요 없이, **충분히 확실하면** 중간 레이어에서 exit합니다.
