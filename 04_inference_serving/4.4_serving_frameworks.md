...
> LLM을 production 환경에서 효율적으로 서빙하는 주요 프레임워크들.

---

## vLLM

UC Berkeley에서 개발한 LLM serving 프레임워크. 현재 오픈소스 serving의 표준.
...
HuggingFace의 LLM serving solution.
...
NVIDIA에서 개발한 LLM 추론 최적화 라이브러리. H100 같은 최신 NVIDIA GPU에서 최고 성능을 냄.

### 핵심 기능

**Kernel Fusion**: 여러 GPU 연산을 하나의 커널로 통합함:
```
Without fusion:
...
Language model programming을 위한 structured generation 프레임워크.

### 핵심 기능

**RadixAttention (Prefix Caching)**:
Radix tree를 이용하여 KV-cache를 prefix 단위로 재사용함:
...
Python 비슷한 문법으로 **커스텀 GPU 커널**을 작성하는 언어/컴파일러.
...
Triton은 CUDA보다 훨씬 쉽게 효율적인 GPU 커널을 작성할 수 있게 함.

**FlashAttention-2가 Triton으로 구현**되어 있음.
...
여러 연산을 하나의 GPU 커널로 합쳐서 메모리 접근 횟수를 줄임.
