...
> Vision-Language Model의 다양한 능력을 평가하는 벤치마크들을 소개합니다.
...
MMLU의 multimodal 버전입니다. 대학교 수준의 학문적 질문에 이미지가 포함됩니다.
...
**특징**: 단순 이미지 인식이 아닌 **domain knowledge + visual reasoning** 조합이 필요합니다.
...
**특징**: 능력별 breakdown이 제공되어 모델의 강약점 분석에 유용합니다.

### MMStar

**오염 방지** 설계 원구로, 이미지 없이 텍스트만으로도 답할 수 있는 문제는 제외합니다:
...
**이미지 내의 텍스트를 읽고 이해**하는 능력을 테스트합니다:
...
OCR(Optical Character Recognition)과 visual reasoning을 조합합니다.

**현재 SOTA**: 80%+ (GPT-4V, Qwen2-VL 등)

### DocVQA

**문서 이미지**(스캔된 문서, 양식, 인보이스 등)에서 정보를 추출합니다:
...
VLM의 **Object Hallucination**을 평가합니다.

### 방법

이미지에 **없는 물체**에 대해 "이 이미지에 X가 있나요?" 라고 묻습니다:
...
**의의**: POPE 점수가 낮으면 모델이 이미지를 제대로 보지 않고 언어 prior에 의존한다는 의미입니다.
