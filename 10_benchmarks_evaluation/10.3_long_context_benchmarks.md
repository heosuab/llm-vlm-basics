...
> 모델이 긴 입력을 제대로 처리하는지 평가하는 벤치마크들.
...
매우 긴 텍스트("haystack") 중간에 숨겨진 짧은 정보("needle")를 찾아내는 능력을 테스트함.
...
좋은 모델은 **모든 context length, 모든 위치에서 100%** 가까운 점수를 냄.

### 발견

초기 long-context 모델들에서 **"Lost in the Middle"** 현상이 발견됨:
- 처음과 끝 부분에 needle이 있으면 잘 찾음
- **중간 부분에 needle이 있으면 자주 놓침**

이후 모델 개선(Gemini 1.5, LLaMA-3.1 등)으로 대부분의 위치에서 높은 성능을 보임.

### Multi-Needle NIAH

하나가 아닌 여러 개의 needle을 동시에 찾는 더 어려운 버전.
...
NIAH는 단순 recall만 테스트함. **RULER (Realistic Understanding for Long-context Evaluation Resources)**는 더 다양하고 현실적인 long-context 능력을 평가함.
...
긴 텍스트에서 변수가 여러 번 업데이트될 때 최종 값을 추적함:
...
긴 텍스트에서 가장 자주 등장하는 단어들을 찾음 — 통계적 집계 능력 테스트.

**4. Frequent Words Extraction (FWE)**:
여러 목록에서 공통으로 등장하는 항목들을 찾음.

**5. Question Answering (QA)**:
여러 문서에 분산된 정보를 종합하여 답변함 (multi-hop reasoning).

### 의의

RULER는 모델이 long context를 **실제로 처리하는지** (진짜 long-context understanding) vs **단순히 처음/끝을 보는지**를 구분함.
...
실제 downstream task들에 대한 long-context 평가:
...
100K+ token의 극단적으로 긴 context에 대한 평가. 기존 벤치마크가 포화될 때 더 어려운 테스트를 제공함.
...
Long-context 능력은 **용량**(얼마나 긴 context를 처리할 수 있는가)과 **품질**(그 context를 얼마나 잘 이해하고 활용하는가) 두 가지를 모두 고려해야 함.
