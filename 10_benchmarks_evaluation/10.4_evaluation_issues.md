...
> 벤치마크를 올바르게 해석하기 위해 알아야 할 중요한 문제들입니다. Research engineer라면 이 문제들을 항상 인식하고 있어야 합니다.
...
모델의 학습 데이터에 벤치마크의 test set이 포함되어 있으면, 모델이 답을 "암기"한 것인지 "이해"한 것인지 구분할 수 없습니다.
...
가장 널리 사용되는 LLM evaluation 프레임워크입니다.
...
**중요성**: 같은 모델도 평가 방법(shot 수, prompt 형식, normalization)에 따라 점수가 다르게 나옵니다. 표준 harness를 사용해야 비교가 공정합니다.
...
LLM을 사용하여 다른 LLM의 출력을 평가하는 방법입니다. 비용이 낮고 확장 가능하지만, 여러 편향이 있습니다.

### Position Bias (순서 편향)

같은 두 응답이어도 A/B 순서에 따라 다르게 판단합니다:
...
모델들이 벤치마크의 "ceiling"에 근접하여, 더 이상 성능 차이가 구분되지 않습니다.
