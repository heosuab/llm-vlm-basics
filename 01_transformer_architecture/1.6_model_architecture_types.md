...
> LLM의 대표적인 아키텍처 유형들과 각각의 특성, 적합한 사용 사례를 정리합니다.
...
입력 시퀀스 전체를 **양방향(bidirectional)**으로 처리합니다.
모든 token이 왼쪽과 오른쪽 모두를 볼 수 있습니다.
...
각 token의 output이 해당 위치의 문맥 표현(contextual representation)입니다.
...
전체 token의 ~15%를 랜덤으로 masking하고 예측합니다.
양방향 context를 활용하여 더 풍부한 표현 학습이 가능합니다.
...
**생성 불가**: autoregressive generation에 적합하지 않습니다.
LLM의 주류가 decoder-only로 넘어가면서 사용이 줄었지만, 임베딩/분류 task에서는 여전히 활용됩니다.
...
Causal (left-to-right) attention만 사용합니다.
각 token은 자신과 이전 token들만 볼 수 있습니다.
...
모든 position에서 next token을 예측합니다.
하나의 시퀀스가 n개의 학습 예시를 제공합니다 → 데이터 효율적
...
Encoder가 입력을 처리하고, Decoder가 출력을 생성합니다.
Decoder는 encoder output을 cross-attention으로 참조합니다.
...
LLM 시대에서는 GPT 계열에 밀렸지만, 번역·요약 등 특정 seq2seq task에서는 여전히 강점이 있습니다. T5는 instruction tuning 연구(FLAN-T5)에 많이 활용되었습니다.
...
입력의 "prefix" 부분은 **양방향** attention, 이후 생성 부분은 **단방향** attention으로 처리합니다.
...
Prefix 내에서는 모든 token이 서로를 볼 수 있어 더 풍부한 표현을 얻을 수 있고, 이후 생성 부분은 일반 decoder처럼 작동합니다.
...
FFN 레이어를 여러 개의 "expert" FFN으로 교체하고, 각 token에 대해 **상위 k개의 expert만 활성화**합니다.
...
같은 추론 비용으로 4배 더 많은 파라미터를 갖는 셈입니다.
...
특정 expert에만 token이 몰리는 "expert collapse" 현상을 방지합니다:
...
이 loss를 메인 loss에 더해서 모든 expert가 균등하게 사용되도록 유도합니다.

---

## Capacity Factor

각 expert가 처리할 수 있는 최대 token 수를 제한하는 파라미터입니다.
...
Capacity factor가 너무 작으면 token drop이 많이 발생하고, 너무 크면 메모리 낭비가 됩니다.

**DeepSeek-V2의 혁신**: Top-k routing에서 expert group을 만들어 inter-node communication을 최소화하는 효율적인 MoE routing을 도입했습니다.
