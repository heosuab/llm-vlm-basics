...
**LLM에서의 추세**: 대규모 LLM 사전학습에서는 dropout을 거의 사용하지 않는 추세. 데이터가 충분히 많고 training time이 짧아 overfitting이 문제가 되지 않기 때문.
...
> - 오해의 소지 정정: 모델 규모 자체가 커지다보니 전체에서 embedding/LM head가 차지하는 비중이 상대적으로 작아서, 그걸 굳이 묶어 얻는 이득보다 untied의 자유도를 택하는 경우가 있다 (최신 대형 모델 일부는 그 절약보다 입력 임베딩과 출력 LM head를 분리했을 때의 유연성을 더 높게 본다는 뜻)
> - 입력 임베딩은 "단어의 의미"를 배우는 데 집중하고, 출력층은 "다음 단어로 무엇이 올지(확률)"를 결정하는 데 집중 (input embedding과 output classifier의 역할이 다르기 때문에 따로 두면 표현의 자유도가 늘어남)
