...
> 표준 Multi-Head Attention(MHA)의 메모리·속도 문제를 해결하기 위한 다양한 변형들을 다룹니다.
...
표준 MHA에서 h개의 head가 각각 독립적인 K, V projection을 가집니다.
MQA는 **모든 head가 단 하나의 K와 V를 공유**합니다.
...
PaLM이 처음 도입했고, Falcon 등에서 사용합니다.

---

## Grouped Query Attention (GQA)

MHA(각 head 독립)와 MQA(단 1개 공유) 사이의 **중간점**입니다.
G개의 그룹으로 나눠서, 그룹 내에서 K, V를 공유합니다.
...
GQA는 현재 decoder-only LLM의 사실상 표준이 되었습니다.

---

## Sparse Attention

모든 n² 쌍을 계산하지 않고 **선택된 쌍만** 계산합니다.

### 패턴 유형

**Local (window) attention**: 각 token은 주변 w개의 token만 봅니다
```
token i attends to: [i-w/2, ..., i+w/2]  (bidirectional)
                또는: [i-w, ..., i]         (causal)
```

**Strided attention**: 규칙적인 간격으로 떨어진 token들을 봅니다
```
...
Local attention의 실용적 구현입니다. 각 token이 자신을 중심으로 **w개의 window 내의 token들만** attend합니다.
...
Softmax attention을 **kernel function으로 근사**하여 O(n²)을 O(n)으로 줄입니다.
...
현재는 연구 단계이며, 실제 프로덕션 LLM에는 거의 사용되지 않습니다.

---

## FlashAttention (v1/v2/v3)

FlashAttention은 attention 연산 자체를 바꾸지 않고 **IO를 최적화**하여 빠르게 만듭니다.
...
FlashAttention은 현재 LLM 학습·추론의 표준 구현입니다.

---

## Memory-Efficient Attention Kernels

FlashAttention 외에도 메모리 효율적인 attention 구현들이 있습니다.
...
**실용 팁**: PyTorch 2.0+에서는 `torch.nn.functional.scaled_dot_product_attention()`이 자동으로 FlashAttention을 사용합니다.
