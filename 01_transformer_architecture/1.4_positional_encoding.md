...
> Transformer는 순서를 모르는 구조이므로, 위치 정보를 별도로 주입해야 함. 이를 위한 positional encoding의 방법들과 그 장단점.

---

## 왜 Positional Encoding이 필요한가?

Self-attention은 "어떤 위치에 있는지"에 관계없이 token 간의 관계만 계산.
즉, "The dog bit the man"과 "The man bit the dog"을 동일하게 처리.

위치 정보를 주입해야 순서를 학습할 수 있음.

---

## Sinusoidal Positional Encoding

원래 "Attention Is All You Need" 논문에서 제안된 방법.
...
**직관**: 각 차원이 서로 다른 주파수의 sin/cos 파형으로 위치를 인코딩함.
작은 i일수록 파장이 짧고 (세밀한 위치 구분), 큰 i일수록 파장이 긺 (대략적 위치).
...
현대 LLM의 표준 positional encoding. LLaMA, Mistral, Qwen, Gemma 등이 모두 사용함.

### 핵심 아이디어

위치 정보를 embedding에 더하는 것이 아니라, **Q와 K 벡터를 위치에 따라 회전**시킵니다.
그 결과, Q와 K의 내적(attention score)이 **상대 위치(relative position)에 의존**하게 됨.
...
즉, attention score가 m-n(상대 위치)에만 의존하게 됨.
...
position encoding 대신 **attention score에 선형 페널티를 직접 추가**함.
...
RoPE의 "Neural Tangent Kernel" 관점에서 base frequency를 조정하여 context length를 늘리는 방법.
...
학습 시 최대 길이(예: 4K) 이상에서 RoPE를 사용하는 방법들.

### Position Interpolation (PI)
긴 위치를 그냥 쓰는 대신, 기존 범위 내로 **압축(interpolate)**함:
```
position = pos * (train_len / new_len)
```
예: train_len=4096, new_len=32768이면, position을 8배 압축.
소량의 fine-tuning으로 좋은 성능을 냄.

### YaRN (Yet another RoPE extensioN)
NTK scaling + 고주파 성분 보존을 결합:
- 고주파 성분 (가까운 거리): 변경 없음 (이미 잘 작동)
- 저주파 성분 (먼 거리): interpolation 적용
LLaMA-2의 long context 버전에 사용됨.

---

## 2D Positional Encoding (Vision)

이미지 patch를 sequence로 처리할 때, 1D PE를 2D로 확장함.

### 방법 1: 분리 가능(Factorized) 2D PE
```
PE(row, col) = PE_row(row) + PE_col(col)
```
행과 열을 독립적으로 인코딩하고 더함.

### 방법 2: 2D Sinusoidal PE
2D 좌표를 각각 sin/cos로 인코딩하여 concat함.

### 방법 3: Learned 2D PE
각 (row, col) 쌍에 대한 임베딩을 직접 학습함 (ViT 기본 방식).

---

## Multimodal RoPE (M-RoPE)

Qwen2-VL에서 도입된 방법으로, 텍스트와 이미지 token에 **별도의 position ID**를 할당함.
