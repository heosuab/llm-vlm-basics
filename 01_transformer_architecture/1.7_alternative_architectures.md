...
> Transformer의 O(n²) 복잡도 한계를 극복하기 위한 대안 아키텍처들을 소개합니다. 2024년부터 실용화가 빨라지고 있습니다.
...
대안 아키텍처들은 이 문제들을 O(n) 복잡도로 해결하려 합니다.
...
SSM은 연속 시간 동적 시스템(continuous-time dynamical system)에서 유래한 모델입니다.
...
A 행렬을 랜덤이 아닌 수학적으로 의미있는 초기값으로 설정합니다.
"High-order Polynomial Projection Operators"에서 유래하며, 긴 range의 의존성을 포착하는 데 유리합니다.

---

## Mamba / Mamba-2 (Selective SSM)

2023년 Albert Gu, Tri Dao가 제안. 현재 가장 널리 사용되는 non-transformer 아키텍처입니다.

### 기존 SSM의 문제

고전 SSM은 A, B, C가 입력과 무관한 고정값입니다. → **모든 입력에 동일한 방식으로 반응**, 중요한 정보에 집중하지 못함

### Mamba의 혁신: Selective State Space

**A, B, C를 입력 x에 따라 동적으로 결정합니다:**
...
이를 통해 "관련 있는 정보는 기억하고, 관련 없는 정보는 잊는" 선택적 처리가 가능합니다.
...
Mamba의 이론적 기반을 강화하고 더 큰 state dimension을 효율적으로 처리합니다.
...
Transformer와 RNN의 장점을 결합한 "RNN-Transformer hybrid" 모델입니다.
...
순수 SSM 모델의 한계를 극복하기 위해 **SSM layer + Attention layer를 혼합**합니다.
...
Transformer가 여전히 지배적이지만, 긴 시퀀스와 효율적인 추론이 중요해지면서 non-transformer 아키텍처의 중요성이 계속 높아지고 있습니다.
