...
> Attention 메커니즘의 수식과 구현 세부사항.

---

## Scaled Dot-Product Attention

Transformer의 핵심 연산. 전체 수식:
...
d_k 차원의 벡터 Q, K가 각 성분이 평균 0, 분산 1을 가지는 경우, 내적 Q·K의 분산은 **d_k**가 됨.
...
Decoder-only LM(GPT 계열)은 future token을 볼 수 없어야 함. 이를 위해 upper triangular 부분을 `-inf`로 마스킹.
...
위치 0은 자기 자신만, 위치 1은 0과 1, ... 이런 식으로 과거만 볼 수 있음.

---

### Padding Mask

Batch 내의 시퀀스 길이가 다를 때, 짧은 시퀀스는 `[PAD]` token으로 채움. 이 padding position은 attention에 포함되지 않아야 하므로 마스킹.
...
Attention mask와 별개로, 학습 시에는 **어느 위치에서 loss를 계산할지**를 정하는 **loss mask**를 사용.

핵심 차이:

| 구분 | Attention Mask | Loss Mask |
|------|----------------|-----------|
| 역할 | 어떤 토큰을 **볼 수 있는지** 제어 | 어떤 위치를 **채점할지** 제어 |
| 적용 시점 | attention score 계산 시 | logits와 labels로 loss 계산 시 |
| 목적 | 정보 흐름 제한 | 학습 신호 위치 선택 |

즉:
- **attention mask**는 forward 중 정보 접근을 제한
- **loss mask**는 forward 결과 중 일부 위치만 loss에 반영

---

### 왜 loss mask가 필요한가?

Instruction tuning / SFT에서는 보통 입력을 다음처럼 구성한다:
...
이때 모델은 전체 시퀀스에 대해 autoregressive하게 동작하므로,  
attention은 전체 시퀀스에 causal mask를 적용.

하지만 우리가 실제로 학습시키고 싶은 것은 보통:

- question을 맞히는 것 ❌
- **question을 조건으로 answer를 생성하는 것** ⭕

이다.

그래서 loss는 보통 **answer 영역에만 계산**.
...
Long Context를 다루는 것이 어려운 근본 원인.

---

## KV-Cache Mechanics

Autoregressive generation에서 매 step마다 전체 시퀀스에 대한 attention을 다시 계산하면 비효율적. KV-cache는 이를 해결하고자 하는 방법.
...
이전 step에서 계산한 Key, Value를 cache에 저장:
