...
Vision Encoder가 이미지를 벡터로 변환했다면, 이제 그 벡터를 Language Model이 이해할 수 있는 형태로 연결해야 합니다. 이 연결 방식(Connector/Projector)의 선택이 VLM의 성능, 효율성, 그리고 능력에 큰 영향을 미칩니다. 이 섹션에서는 다양한 연결 방식과 융합(Fusion) 전략을 깊이 살펴봅니다.
...
이 아키텍처의 핵심 장점은 **모듈성(Modularity)**입니다. 각 구성 요소를 독립적으로 업그레이드하거나 교체할 수 있습니다.

### 1.2 Vision Encoder의 동결/파인튜닝 선택

Vision Encoder를 어떻게 처리할지는 중요한 설계 결정입니다:
...
**현재 추세**: 초기 학습 단계에서는 동결, 후기 단계에서는 낮은 학습률로 파인튜닝하는 **2단계 전략**이 일반적입니다.
...
Linear Projector는 Vision Encoder의 출력과 LLM 입력 공간 사이에 단순한 선형 변환을 적용합니다:
...
2023년 발표된 LLaVA (Large Language and Vision Assistant)의 첫 번째 버전은 이 단순한 Linear Projector를 사용해 놀라운 성능을 보였습니다. "단순함이 때로는 최선"이라는 것을 증명한 사례입니다.
...
MLP Projector는 Linear Projector에 비선형 활성화 함수와 추가 레이어를 더해 표현력을 높입니다:
...
LLaVA-1.5 논문에서 보고된 결과에 따르면, 2-layer MLP Projector는 Linear Projector보다 다양한 VQA 및 멀티모달 벤치마크에서 일관되게 우수한 성능을 보입니다. 특히 세밀한 시각 이해가 필요한 태스크에서 차이가 뚜렷합니다.
...
MLP Projector는 현재 많은 효율적인 VLM의 표준 선택입니다. 단순함, 낮은 계산 비용, 좋은 성능의 균형을 제공합니다. LLaVA-1.5 이후 MLP Projector가 사실상 기본값이 되었습니다.
...
Q-Former는 BLIP-2와 InstructBLIP에서 도입된 더 복잡한 연결 모듈입니다. 핵심 아이디어는 **고정된 수의 학습 가능한 Query Token**이 Image Feature에서 정보를 추출한다는 것입니다.
...
이미지 해상도나 패치 수에 관계없이 항상 **K=32개의 고정된 시각 토큰**만 LLM에 전달됩니다.
...
BLIP-2는 Q-Former를 효율적으로 학습하기 위해 3단계 학습을 도입했습니다:
...
Perceiver Resampler는 DeepMind의 Flamingo 모델에서 도입된 연결 모듈입니다. Q-Former와 매우 유사하지만 몇 가지 차이점이 있습니다:
...
**Gated Cross-Attention**: Flamingo에서 LLM의 각 레이어에 시각 정보를 주입하는 메커니즘입니다. 원래 LLM 레이어 사이에 새로운 Cross-Attention 레이어를 삽입하고, Gate 파라미터로 그 영향도를 조절합니다:
...
처음에 `gate=0`이면 원래 LLM과 동일하게 동작하다가, 학습을 통해 점점 시각 정보를 더 활용합니다.
...
시각 토큰과 텍스트 토큰을 **초기 단계에서 합쳐** 함께 처리합니다:
...
각 모달리티를 **독립적으로 처리**한 뒤 최종 단계에서 결합합니다:
...
실제로는 Early와 Late 사이 어딘가에서 융합하는 경우도 많습니다:
...
Unified Multimodal Transformer는 처음부터 텍스트와 시각 토큰을 **동일한 Transformer로 함께 처리**합니다. 별도의 Vision Encoder나 Connector 없이, 단일 Transformer가 모든 모달리티를 처리합니다.
...
Unified Transformer에서 가장 큰 도전은 서로 다른 모달리티에 대한 위치 정보 인코딩입니다:
...
**현재 동향**: Unified 접근은 GPT-4o, Gemini와 같은 최첨단 상업용 모델들이 지향하는 방향입니다. 오픈소스에서는 아직 Connector 기반이 주류이지만, Unified 방식으로의 이행이 진행 중입니다.
...
이 구조에서 LLM은 시각 토큰을 "그림 단어"처럼 처리합니다. Projector가 시각 특징을 LLM의 Token Embedding 공간으로 매핑했기 때문에, LLM 입장에서 시각 토큰과 텍스트 토큰의 형태가 동일합니다.
