...
VLM을 학습시키는 것은 단순히 Vision Encoder와 LLM을 연결하는 것 이상입니다. 실제로 고품질의 VLM을 만들기 위해서는 여러 까다로운 학습 문제들을 해결해야 합니다. 이 섹션에서는 Vision-Text Alignment, Hallucination, Grounding, 멀티이미지/비디오 처리, 그리고 Catastrophic Forgetting까지 VLM 학습의 핵심 문제들을 다룹니다.
...
Vision Encoder(예: CLIP ViT)와 LLM(예: LLaMA)은 완전히 **다른 방식으로 사전학습**되었습니다:

- **Vision Encoder**: 이미지-텍스트 대조 학습 → 이미지와 텍스트를 임베딩 공간에서 정렬
- **LLM**: 다음 토큰 예측(Causal Language Modeling) → 텍스트 시퀀스의 분포를 모델링

이 두 공간은 완전히 다른 특성을 가지고 있습니다. 시각 특징을 LLM의 토큰 임베딩 공간으로 매핑하는 것이 바로 **Alignment**의 핵심입니다.

### 1.2 2단계 학습 파이프라인 (Two-Stage Training Pipeline)

현재 가장 검증된 방법은 LLaVA가 확립한 **2단계 학습 전략**입니다:
...
이 단계에서 Projector는 "시각 특징을 어떻게 LLM이 이해하는 형태로 변환할지"를 학습합니다. 마치 번역가처럼 시각 언어를 텍스트 언어로 번역하는 능력을 키우는 것입니다.
...
VLM의 Hallucination은 **이미지에 없거나 부정확한 정보를 사실인 것처럼 생성**하는 문제입니다. LLM의 Hallucination과 비슷하지만, 이미지라는 추가적인 그라운딩 소스가 있음에도 틀린 정보를 생성한다는 점에서 더 심각합니다.

### 2.2 Object Hallucination (객체 환각)

모델이 이미지에 **존재하지 않는 물체를 있다고 주장**하는 경우입니다:
...
물체는 맞게 인식하지만 **색상, 크기, 개수 등의 속성을 잘못 설명**하는 경우입니다:
...
POPE는 VLM의 Object Hallucination을 측정하는 표준 벤치마크입니다:
...
Adversarial POPE가 가장 어렵습니다. 모델이 Language Prior에 의존한다면 "컵이 있으니 접시도 있을 것"이라는 환각을 일으킵니다.
...
Grounding은 **언어 표현(Language Expression)을 이미지의 특정 영역(Region)**에 연결하는 능력입니다. 단순히 이미지를 설명하는 것을 넘어, "어디에" 있는지를 파악합니다.
...
이미지를 설명할 때 각 설명이 이미지의 어느 부분과 연결되는지를 명시합니다:
...
많은 실제 사용 사례가 여러 이미지를 동시에 처리해야 합니다:
...
멀티이미지 처리는 개념적으로 단순합니다: 각 이미지를 독립적으로 Vision Encoder로 처리한 뒤, 시각 토큰들을 텍스트 시퀀스에서 각자의 자리에 삽입합니다:
...
중요한 기술적 문제는 **여러 이미지에 걸친 위치 인코딩**입니다:
...
**Qwen2-VL의 M-RoPE**: 이미지에는 2D 위치(행, 열), 텍스트에는 1D 위치를 각각 인코딩하여 혼용하는 방식으로 이 문제를 우아하게 해결합니다.

### 4.4 실용적 제한

현재 대부분의 VLM이 멀티이미지를 지원하지만 실용적 제한이 있습니다:
...
비디오는 기본적으로 **시간 순서로 나열된 이미지(Frame)**의 시퀀스입니다:
...
모든 프레임을 처리하는 것은 불가능하므로, 적절한 프레임을 선택해야 합니다:
...
**Catastrophic Forgetting**은 신경망이 새로운 태스크를 학습할 때 이전에 학습한 능력을 잊어버리는 현상입니다. VLM에서는 특히 두 가지 형태로 나타납니다:
