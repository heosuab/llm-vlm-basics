...
- 학습 동안 Vision encoder는 freeze, LLM도 Perceiver Resampler 외에는 freeze
...
**InternVL 2 (2024)**:
- **Multi-scale Vision Features**: 여러 레이어의 시각 특징을 결합 (ViT도 CNN처럼 layer에 따라 계층적으로 다른 feature가 뽑힘)
...
- scale을 "fusion으로 외부에서 강제"하는 대신 scale representation을 "학습으로 내부화"하는 방향 (Multi-scale Vision Features 없이도 이런 feature를 모델이 알아서 뽑도록 학습되어짐)
