# 7.1 LLM model family 종합 Survey
...
OpenAI의 GPT 패밀리는 현대 LLM 패러다임의 시작점. 각 세대는 이전 세대가 해결하지 못했던 문제를 새로운 방식으로 돌파함.
...
2018년 이전까지 NLP 모델들은 주로 task-specific하게 설계됨. 감성 분류, 기계 번역, QA 등 각 태스크마다 별도의 아키텍처와 학습 데이터가 필요했음. GPT-1은 이 패러다임을 깨고 **"하나의 pretrained model을 여러 downstream task에 fine-tuning하면 된다"**는 개념을 대규모로 입증함.

**핵심 아키텍처 혁신**

- **Decoder-only Transformer**: 기존 Transformer (Vaswani et al., 2017)는 encoder-decoder 구조였으나, GPT-1은 decoder만 사용하는 causal language model을 채택함. 이로써 left-to-right autoregressive 생성이 자연스럽게 가능해짐.
...
- **Pretraining + Fine-tuning 패러다임**: 대규모 unlabeled text로 language model pretraining 후, labeled downstream data로 supervised fine-tuning. Fine-tuning 시 language modeling objective를 auxiliary loss로 함께 사용하여 일반화 성능을 높임.
- **Task-specific input transformation**: 분류/유사도/QA 등 다양한 task의 입력을 Transformer가 처리할 수 있는 단일 token sequence로 변환하는 체계적인 방법을 제시함.

**이전 모델 대비 해결한 문제**

이전의 word2vec, ELMo 등은 contextual representation을 제공했으나, 각 task마다 독립적인 fine-tuning 구조가 필요했음. GPT-1은 단일 pretrained backbone을 다양한 NLP task에 통일된 방식으로 적용하는 것이 실제로 작동함을 증명함.
...
GPT-1의 성공 이후, OpenAI는 "더 큰 모델, 더 많은 데이터를 쓰면 어떻게 될까?"라는 질문을 탐구함. GPT-2는 **스케일링이 zero-shot 성능과 emergent capability를 만들어낸다**는 사실을 처음으로 대중에게 각인시킴. 당시 OpenAI는 이 모델이 "too dangerous to release"라며 단계적으로 공개하여 화제가 됨.
...
GPT-1은 fine-tuning이 필수였으나, GPT-2는 fine-tuning 없이도 상당한 성능을 보여줌으로써 **"pretrained LM 자체가 task solver가 될 수 있다"**는 가능성을 열었음. 또한 스케일링이 체계적으로 성능을 높인다는 경험적 증거를 제공함.
...
GPT-3는 NLP 역사의 분기점. **175B 파라미터**라는 전례 없는 스케일로, few-shot in-context learning이라는 새로운 패러다임을 확립함. "Brown et al., 2020: Language Models are Few-Shot Learners" 논문은 AI 역사에서 가장 영향력 있는 논문 중 하나가 됨.
...
- **In-context learning (ICL)**: 모델 파라미터를 업데이트하지 않고, 프롬프트에 몇 가지 예시(few-shot examples)를 제공함으로써 새로운 task를 수행. 이는 gradient update 없는 "meta-learning"이라 볼 수 있음.
  - **Zero-shot**: task description만 제공
  - **One-shot**: 1개의 예시 제공
  - **Few-shot**: 여러 예시 제공
- **Scale as the primary lever**: Chinchilla law 이전의 시대로, compute를 주로 모델 크기 증가에 집중했음.

**이전 모델 대비 해결한 문제**

GPT-2가 zero-shot에서 가능성을 보여줬다면, GPT-3는 few-shot에서 fine-tuned smaller model을 능가하는 성능을 달성함. **"Fine-tuning 없이도 실용적인 AI assistant를 만들 수 있다"**는 개념의 실증. OpenAI API를 통해 상업적 서비스가 시작되었고, 이후 LLM 산업 전체의 틀이 잡혔음.
...
GPT-3는 강력했으나 **alignment 문제**가 있었음: 모델이 유저의 의도와 다른 방향으로 응답하거나, 유해한 내용을 생성하거나, 단순히 프롬프트를 "완성"하려 했음. InstructGPT는 **RLHF (Reinforcement Learning from Human Feedback)**를 통해 이 문제를 해결하고, "instruction following"을 가르쳤음.
...
ChatGPT는 새로운 기술이라기보다는 **InstructGPT의 대화(dialogue) 최적화 버전**. 그러나 2022년 11월 출시 후 사상 최단 기간(5일)에 100만 명 사용자를 돌파하며, **AI의 대중화**를 이끈 역사적 제품이 됨.
...
API를 통해서만 접근 가능했던 GPT-3와 달리, 일반인이 웹 인터페이스로 쉽게 사용할 수 있게 됨. 코딩 도움, 글쓰기, 분석 등 실용적 use case를 실증함으로써 "LLM은 연구용이 아니라 실용 도구"임을 입증함.
...
GPT-4는 OpenAI가 "GPT-4 Technical Report"를 공개했으나 **파라미터 수, 아키텍처 세부 사항을 비공개**로 유지한 최초의 주요 모델. 다양한 professional exam(변호사, 의사 시험 등)에서 인간 수준을 넘어섰으며, **multimodal** 입력(이미지 + 텍스트)을 처음으로 지원함.

**핵심 아키텍처 혁신**

- **Multimodal input**: 텍스트와 이미지를 함께 입력으로 처리 (비전 인코더 통합)
- **Mixture of Experts (MoE)** (비공식 루머 / 분석): 업계에서 GPT-4가 sparse MoE 아키텍처를 사용한다는 분석이 많음. 약 8개의 expert, 각 토큰마다 2개 activation으로 추정. 공식 확인 없음.
...
GPT-4o ("o"는 "omni")는 **native multimodal** 모델로, 텍스트/오디오/이미지/비디오를 단일 모델 내에서 end-to-end로 처리함. 이전의 음성 기능이 ASR → LLM → TTS 파이프라인이었다면, GPT-4o는 오디오 토큰을 직접 처리하여 감정, 톤, 속도 등을 이해함.
...
OpenAI o1 (2024년 9월)은 LLM 역사에서 **패러다임 전환점**. 기존 모델들이 "더 많은 training compute"로 발전했다면, o1은 **inference-time compute scaling**이라는 새로운 축을 도입함. 모델이 답변하기 전에 내부적으로 긴 "thinking" 과정을 거침.
...
- 아키텍처 자체는 비공개이나, 사고 과정이 structured되고 반성(reflection)을 포함함
...
LLaMA 패밀리는 **"open-weights LLM의 민주화"**를 이끈 가장 중요한 시리즈. Meta의 공개로 인해 학술 연구자, 스타트업, 개인 개발자들이 frontier-class 모델을 활용할 수 있게 됨.
...
GPT-3, PaLM 등 대형 LLM들은 모두 closed-source였음. Meta AI Research는 **"compute-optimal training"** (Chinchilla 논문의 insight)을 적용하여, 파라미터 수를 줄이고 더 많은 데이터로 학습하면 더 효율적임을 실증함. 특히 **7B LLaMA-1이 GPT-3 (175B)를 일부 벤치마크에서 능가**한 것은 충격.
...
LLaMA-1은 연구 목적으로만 사용 가능했으나, LLaMA-2는 **상업적 라이선스**를 포함하여 공개됨. 또한 Chat 버전(RLHF 적용)도 함께 공개하여 GPT-3.5/ChatGPT의 실질적 대안이 됨.
...
LLaMA-3 시리즈는 Meta가 단순한 "연구 공개"를 넘어 **frontier model과 경쟁**하겠다는 의지를 보여줌. LLaMA-3.1 405B는 GPT-4 수준의 성능을 open-weights로 달성한 최초의 모델 중 하나.
...
Mistral AI는 2023년 설립된 프랑스 스타트업으로, **효율적인 아키텍처 혁신**으로 빠르게 주목받음. 특히 Sliding Window Attention과 Mixture of Experts를 조합하여 소형 모델의 한계를 돌파함.
...
출시 당시 가장 강력한 7B 오픈소스 모델. LLaMA-2 13B보다 뛰어난 성능을 7B 파라미터로 달성하며, **아키텍처 최적화가 단순 스케일링보다 중요할 수 있다**는 것을 입증함.
...
Mixtral 8x7B는 **오픈소스 MoE (Mixture of Experts) 모델의 표준**을 세움. 총 46.7B 파라미터이지만 실제 추론 시 **12.9B만 활성화**되어, 13B 모델 수준의 추론 비용으로 70B 수준의 성능을 달성함.
...
Mistral의 첫 번째 진정한 frontier-class 모델. **123B 파라미터**로 GPT-4와 경쟁하는 수준의 성능을 제공하며, 특히 코딩과 다국어에서 강점을 보임.
...
2025년 초 출시된 **24B 파라미터** 모델로, "small but capable"의 포지셔닝. 로컬 배포 및 비용 효율적인 서비스를 위해 설계됨.
...
Google/DeepMind는 LLM 연구에서 Transformer 자체를 발명한 기관으로, 다양한 아키텍처적 혁신을 지속해왔음.
...
PaLM (Pathways Language Model, 2022)은 **540B 파라미터**로 당시 최대 규모 LLM 중 하나. Google의 Pathways 인프라를 활용하여 6,144개의 TPU chip에서 학습됨. PaLM-2 (2023)는 Gemini 전 세대의 최후 대형 모델로, Bard(현 Gemini)의 기반이 됨.
...
Gemma는 Google DeepMind의 오픈소스 LLM 시리즈. Gemini와 같은 연구 및 기술을 소형 모델에 적용하여 공개함. Gemma 2는 특히 **knowledge distillation**과 새로운 attention 기법을 도입함.
...
Gemma 3는 **최초의 multimodal Gemma** 시리즈로, 텍스트와 이미지를 함께 처리함. 1B부터 27B까지 다양한 크기를 제공하며, 특히 single-GPU 로컬 배포에 최적화되어 있음.
...
Gemini는 Google DeepMind의 **flagship native multimodal** 모델 시리즈. 처음부터 텍스트, 이미지, 오디오, 비디오를 통합 처리하도록 설계되었으며, Gemini 1.5에서 **1M token context window**를 달성함.

**Gemini 1 (2023년 12월) 핵심 혁신**

- **Native multimodal from scratch**: 별도 비전 인코더가 아닌, 처음부터 모든 모달리티를 통합 학습
- **Ultra, Pro, Nano** 세 가지 크기 변형
- Gemini Ultra: GPT-4 수준의 성능, 최초로 MMLU에서 인간 전문가 수준(90%+) 달성함
...
Qwen (通义千问)은 Alibaba Cloud가 개발한 LLM 시리즈로, **중국어와 영어 이중 언어**에서 강한 성능을 보이며 최근에는 글로벌 frontier model과 경쟁하는 수준에 이르렀음.
...
Qwen 시리즈는 중국어 NLP에서 출발했지만, Qwen2.5에 이르러 **수학(Math), 코딩(Coder), 과학(Science)에서 세계 최고 수준**의 specialized 버전들을 내놓음. 0.5B부터 72B까지 폭넓은 크기 범위로 다양한 용도에 대응함.
...
QwQ-32B-Preview는 Qwen 팀이 개발한 **reasoning/thinking model**로, OpenAI o1처럼 긴 chain-of-thought 추론 과정을 거쳐 수학/과학 문제를 해결함. 오픈소스 reasoning model의 첫 주요 사례 중 하나.
...
Qwen2.5-Max는 Alibaba의 **frontier-class MoE 모델**로, GPT-4o, DeepSeek-V3 등 최상위 모델과 경쟁하는 수준. 세부 파라미터 수는 비공개이나 대규모 MoE 아키텍처로 추정됨.
...
#### 1) 모델 정체: 네이티브 멀티모달 + 효율형 대형 MoE
- 대표 오픈웨이트: **Qwen3.5-397B-A17B**
  - **총 397B 파라미터**, **토큰당 활성(active) 17B**만 사용하는 **sparse MoE**
  - “거대하지만 추론 비용은 활성 expert 중심으로 제한”하는 설계 방향
- **텍스트/이미지/비디오**를 early-fusion(초기 융합) 방식으로 함께 학습한 **통합 VLM**을 강조

---

#### 2) 아키텍처 핵심: Gated DeltaNet(선형/리니어 어텐션 계열) + MoE
공식 모델카드에 명시된 블록 구성:

- **Network**: *Hybrid Mixture-of-Experts with Gated DeltaNet*
- **Layers**: 60
- **레이아웃(공개값 요약)**  
  - `15 × (3 × (Gated DeltaNet → MoE) → 1 × (Gated Attention → MoE))`
  - DeltaNet(선형 어텐션 성격) 블록을 주로 쌓고, 주기적으로 일반 attention도 섞는 하이브리드

#### Gated DeltaNet (공개값)
- V용: **64 linear-attn heads**
- QK용: **16 heads**
- head dim: **128**
...
#### MoE 라우팅 (공개값)
- Experts: **512**
- 토큰당 활성: **10 routed + 1 shared**

---

#### 3) 컨텍스트/롱컨텍스트: 262k 네이티브 + 1M 확장(YaRN)
- **Native context length**: **262,144 tokens (~256K)**
- **확장**: YaRN RoPE scaling으로 **~1,010,000 tokens**까지 확장 가능(모델카드 명시)
- 서빙 예시에도 `--max-model-len 262144` 플래그가 기본으로 등장

---

#### 4) 추론 효율 기능: 빠른 디코딩 + MTP + 스펙큘레이티브
- Together/NVIDIA 쪽에서 **“8.6x–19x faster decoding (vs Qwen3-Max)”** 같은 수치로 효율을 주장
- **Multi-Token Prediction (MTP)** 를 학습/서빙 옵션으로 전면에 둠
- vLLM 예시에서 speculative 관련 설정이 포함

---

#### 5) 멀티모달 I/O 및 서빙 포인트(실무적으로 중요)
HF 모델카드 기준으로 서빙 옵션/운영 힌트:

- vLLM 서빙에서
  - **Tool call 지원**: `--enable-auto-tool-choice`, `--tool-call-parser ...`
  - **Text-only 모드**: `--language-model-only`
    - 비전 인코더/멀티모달 경로를 스킵해 **KV cache에 메모리를 더 할당**하는 방향(롱컨텍스트 운영에 유리)
  - **비디오 프레임 샘플링**을 extra_body로 조절하는 힌트(현재 vLLM 한정 지원이라고 명시)

---

#### 6) RL/에이전트 학습 스케일(“million-agent environments”)
- NVIDIA/Together 모델카드에
  - **“million-agent environments에서 RL 스케일링”**
  - “점진적으로 복잡한 작업 분포” 등의 문구가 등장
- 즉, 단순 VLM이라기보다 **툴 사용/에이전트 워크플로우를 염두에 둔 post-training(RL 포함)**을 강조하는 방향

---

#### 6. DeepSeek Family

DeepSeek (深度求索)은 중국의 hedge fund 계열 AI 연구소로, 2024-2025년 **아키텍처 혁신과 효율적 학습**으로 글로벌 AI 업계에 큰 충격을 줌. DeepSeek-R1의 공개는 특히 미국 AI 주식을 폭락시킬 만큼 파급효과가 컸음.
...
DeepSeek-V2는 **MLA (Multi-head Latent Attention)**라는 새로운 attention 메커니즘을 도입하여 KV cache를 대폭 줄이면서 성능은 유지함. 동시에 MoE 아키텍처로 학습 및 추론 비용을 혁신적으로 절감함.

**핵심 아키텍처 혁신**

**MLA (Multi-head Latent Attention)**:

MLA는 KV cache 문제를 근본적으로 해결하는 혁신적 메커니즘.
...
DeepSeek-V3는 **671B total / 37B active** 파라미터의 대규모 MoE 모델로, 단 **약 560만 달러**의 학습 비용으로 GPT-4o, Claude-3.5-Sonnet 수준의 성능을 달성하여 업계를 충격에 빠뜨림.
...
DeepSeek-R1은 **RLVR (Reinforcement Learning with Verifiable Rewards)**를 통해 GPT-o1에 필적하는 reasoning 능력을 달성한 **오픈 웨이트 reasoning model**. 특히 OpenAI o1과 비슷한 수준의 수학/코딩 성능을 open-source로 공개하여, 미국 AI 주식 폭락과 함께 큰 화제가 됨.
...
R1의 reasoning 능력을 소형 모델에 이식하기 위해, R1이 생성한 long CoT 데이터를 사용하여 Qwen/LLaMA 기반 소형 모델을 distillation함.
...
- **Knowledge distillation from reasoning traces**가 소형 모델에서도 reasoning 능력을 효과적으로 이식함을 입증함
...
Microsoft Research의 Phi 시리즈는 **"quality over quantity"** 원칙으로, **고품질 합성 데이터**로 소형 모델을 학습시키면 대형 모델에 준하는 성능을 낼 수 있음을 증명함.
...
Cohere의 **Command R+** (2024년 4월)은 **RAG (Retrieval-Augmented Generation)** 및 기업용 use case에 최적화된 모델.
...
Anthropic은 OpenAI 전 연구자들이 설립한 회사로, **AI Safety**를 핵심 연구 목표로 삼음. Claude 시리즈는 **Constitutional AI (CAI)**라는 독자적 alignment 방법론을 사용함.
...
UAE의 Technology Innovation Institute (TII)가 개발한 Falcon은 **초기 오픈소스 경쟁력 있는 LLM**의 선구자 중 하나. 2023년 Falcon-40B는 당시 공개 모델 중 최고 수준이었음.
...
**역사적 의미**: LLaMA-1과 함께 오픈소스 LLM 생태계 형성에 기여함. RefinedWeb 데이터 큐레이션 방법론이 이후 데이터 필터링 연구에 영향을 줌.
