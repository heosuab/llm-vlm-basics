...
> LLM의 능력이 형성되는 핵심 단계입니다. 어떤 목표로, 어떤 데이터로 사전학습하는지를 설명합니다.

---

## Next Token Prediction (Causal LM)

현대 LLM의 거의 대부분이 사용하는 pretraining objective입니다.

### 목표

주어진 이전 token들로부터 다음 token을 예측합니다:

```
L = -Σ_t log p_θ(x_t | x_1, ..., x_{t-1})
```

**Teacher forcing**: 학습 시에는 예측한 token이 아닌 **정답 token**을 다음 입력으로 넣습니다. 이를 통해 각 position에서의 예측을 독립적으로 병렬 계산할 수 있습니다.
...
BERT 계열 encoder-only 모델의 pretraining objective입니다.
...
**Next Sentence Prediction (NSP)**: BERT에 포함된 추가 objective였으나, 이후 연구(RoBERTa)에서 별로 도움이 되지 않는다고 밝혀졌습니다.

---

## Span Corruption (T5)

T5(Text-to-Text Transfer Transformer)의 objective입니다.
...
연속된 token span을 sentinel token([X], [Y], ...)으로 대체하고, decoder가 원래 span을 복원합니다.
...
완전히 동일한 문서를 제거합니다. URL 기반 중복이나 hash 기반으로 처리합니다.
...
**SimHash**:
문서를 d-bit hash로 변환하고, Hamming distance가 작은 것들을 near-duplicate로 처리합니다.
...
Raw 웹 데이터에는 저품질 텍스트가 많습니다. 필터링으로 품질을 높입니다.
...
**CCNet, FineWeb**: Common Crawl을 이런 방식으로 정제한 대규모 데이터셋입니다.

---

## Curriculum Learning

쉬운 예시에서 시작해 점점 어려운 예시를 학습하는 전략입니다.
...
LLaMA-3는 학습 마지막 단계에 long context data를 집중적으로 넣어 128K context를 지원합니다.
...
데이터 도메인 비율을 자동으로 최적화하는 방법입니다. Small proxy model로 각 도메인의 중요도를 추정하여 비율을 조정합니다.
