...
> Full fine-tuning의 높은 비용 없이 모델을 특정 task에 적응시키는 방법들입니다.

---

## 왜 PEFT가 필요한가?

7B 모델을 full fine-tuning하려면 약 60GB+ GPU 메모리가 필요합니다 (파라미터 + gradient + optimizer state).
PEFT는 전체 파라미터의 0.1~1%만 업데이트하면서 full fine-tuning에 준하는 성능을 냅니다.
...
2022년 Microsoft에서 제안. 현재 가장 널리 사용되는 PEFT 방법입니다.

### 핵심 아이디어

Weight matrix의 변화량 ΔW를 **두 개의 작은 행렬의 곱으로 근사**합니다:
...
α는 scaling hyperparameter (보통 r과 같거나 2r). r이 바뀌어도 효과적인 learning rate가 유지됩니다.
...
2023년 Tim Dettmers 등이 제안. 70B 모델을 단일 48GB GPU에서 fine-tuning 가능하게 했습니다.
...
Transformer layer 사이에 **작은 bottleneck 모듈을 삽입**합니다.
...
추론 시 추가 레이어를 통과해야 하므로 latency가 약간 증가합니다.

---

## Prefix Tuning

각 Transformer 레이어의 **K, V sequence 앞에 학습 가능한 virtual token을 삽입**합니다.
...
Prefix tuning의 단순화 버전입니다. **입력 embedding 레이어에만** learnable token을 추가합니다.
...
soft_token들은 vocabulary에 없는 **연속적인 벡터**로, gradient로 학습됩니다.

**특이한 점**: 모델이 클수록 prompt tuning의 성능이 full fine-tuning에 가까워집니다. 10B+ 모델에서 효과적입니다.

---

## IA³ (Infused Adapter by Inhibiting and Amplifying Inner Activations)

매우 적은 파라미터(LoRA보다도 적음)로 놀라운 성능을 냅니다.
...
Fine-tuning 후 pre-training에서 학습한 일반 지식이 손상되는 현상입니다.
...
Pre-training에서 중요한 파라미터를 보호합니다:
```
...
Fine-tuning 시 pre-training 데이터를 일부 섞어서 학습합니다:
```
