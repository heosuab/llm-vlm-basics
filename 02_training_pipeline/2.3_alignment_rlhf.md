...
> 모델이 사람의 의도와 가치에 맞게 행동하도록 조정하는 기법들.
...
사람의 선호도(preference)를 수치 점수로 예측하는 모델. RLHF 파이프라인의 핵심 구성 요소.

### 학습 데이터: Pairwise Preference

같은 prompt에 대해 두 가지 응답 중 어떤 것이 더 나은지 사람이 판단함:

```
Prompt: "수도권 여행 추천해줘"
Chosen:  "경복궁, 남산타워, 인사동을 추천함. 각각의 특징은..."  ← 선호
Rejected: "서울 가세요"  ← 비선호
```
...
Reward model은 보통 LLM base에 scalar head를 추가한 형태.

---

## Preference Data Collection

좋은 preference data 없이는 좋은 reward model도, 좋은 RLHF도 없음.
...
2023년에 제안된 방법으로, **Reward model 없이** preference learning을 수행함.
...
DeepSeek의 R1 학습에 사용된 방법으로, **Value model(critic) 없이** PPO를 근사함.

### 핵심 아이디어

각 prompt에 대해 G개의 응답을 샘플링하고, 그 중 상대적인 보상으로 baseline을 추정함:
...
RLVR(Verifiable Rewards)와 결합: 수학 답변 정확성, 코드 실행 결과 등 binary reward를 사용함.

---

## KTO (Kahneman-Tversky Optimization)

**쌍(pair) 없이** 단일 피드백(good/bad)만으로 학습함.
...
**쌍(pair) 데이터**를 사용하지만,  
reference model 없이 log-prob 차이만 직접 최적화.

```
데이터 형식: (prompt, chosen_response, rejected_response)
```

핵심 아이디어:  
chosen과 rejected의 **log-prob 차이**를 직접 최대화.

```
L_SImPO = - log σ( β ( log π(y_w|x) - log π(y_l|x) ) )
```
...
**쌍(pair) 데이터**를 사용하며,  
SFT + preference 학습을 하나의 objective로 통합.
...
핵심 아이디어:  
chosen likelihood를 올리면서 rejected를 내리되,  
**odds ratio 형태**로 안정적 최적화.

```
L_ORPO = L_SFT(chosen)
         - λ log σ( log π(y_w|x) - log π(y_l|x) )
```

- L_SFT: chosen response에 대한 cross-entropy  
- λ: preference 강도 조절  

**특징**
- reference model 없음  
- SFT와 preference를 동시에 수행  
- 학습 안정성 높음  

**장점**: 별도 RLHF 단계 없이 정렬 가능  
...
**특징**
- reference model 필요  
- logistic → regression 형태  
- gradient variance 감소  

**장점**: DPO 대비 안정적이고 튜닝 민감도 낮음  

---

## rDPO (Provably Robust DPO)

DPO를 **label noise에 robust**하도록 확장.
...
핵심 아이디어:  
선호 라벨이 ε 확률로 뒤집힐 수 있다고 가정.

```
Δ = β ( log π(y_w|x) - log π(y_l|x)
        - log π_ref(y_w|x) + log π_ref(y_l|x) )
...
**장점**: noisy preference 데이터에서 안정적  
---

## RLAIF (RL from AI Feedback)

사람 대신 **LLM이 preference를 판단**함.
...
모델 자신이 헌법(constitution)에 따라 자기 응답을 비판하고 수정함.
...
Policy가 reference model(SFT model)에서 너무 멀리 벗어나지 못하게 제약함:
...
**Goodhart's Law**: 측정치가 목표가 되는 순간, 그것은 더 이상 좋은 측정치가 아님.
...
모델의 confidence가 실제 정확도와 일치하는 정도.
...
데이터에 명시적으로 abstention 예시를 포함시켜 SFT로 학습함.
