...
> **학습 목표**: 고품질 학습 데이터를 설계하고 생성하는 방법, 생성된 데이터를 자동으로 검증하는 방법, 그리고 LLM의 평가 및 편향/환각 문제를 깊이 이해한다. 데이터 품질이 모델 성능의 핵심임을 깨닫는다.
...
**Self-Instruct** (Wang et al., 2022)는 초기 seed 예시들에서 점점 더 많은 instruction-response 쌍을 생성하는 방법이다:
...
GPT-4나 Claude 같은 강력한 모델이 더 약한 모델의 학습 데이터를 생성한다:
...
**Self-play**는 모델이 자기 자신과 상호작용하여 데이터를 생성하는 방법이다:
...
**Backtranslation**은 번역 분야에서 시작된 기법으로, 텍스트로부터 역방향으로 instruction을 생성한다:
...
이 방법의 강점: **이미 존재하는 고품질 텍스트를 그대로 활용**하기 때문에 Response 품질이 보장된다.
...
많은 태스크에서 **정답을 만들어내는 것보다 검증하는 것이 훨씬 쉽다**:
...
이 비대칭성을 활용하는 것이 Verification Models의 핵심이다.
...
1. 기본 모델로 여러 풀이를 생성한다
2. 정답 여부를 레이블로 하여 verifier를 **한 번** 학습한다
3. 이후 기본 모델을 개선할 때마다 verifier를 재활용한다
...
강력한 모델을 평가하려면 고품질 인간 평가자가 필요하다. 하지만:
- 비용이 매우 크다
- 확장이 어렵다
- 전문 지식이 필요한 도메인에서 어렵다

**LLM-as-a-Judge**는 강력한 LLM(예: GPT-4)을 평가자로 사용하는 방법이다.
...
LLM-as-a-Judge는 여러 가지 체계적인 편향을 보인다:

#### 1. Position Bias (순서 편향)

같은 응답이라도 **먼저 제시된 응답을 더 높이 평가**하는 경향이 있다.
...
LLM judge는 **더 긴 응답을 더 좋다고 평가**하는 경향이 있다. 실제 품질과 무관하게.
...
LLM은 **자신의 스타일과 유사한 응답을 선호**하는 경향이 있다.
...
Judge LLM이 **사용자/평가자가 선호할 것 같은 방향으로 편향**된 평가를 한다.
...
LLM judge의 신뢰성은 **인간 평가자와의 일치도(agreement)**로 측정한다:
...
**Hallucination**은 LLM이 사실처럼 보이지만 실제로는 틀린 내용을 자신감 있게 생성하는 현상이다. "환각"이라는 이름에 걸맞게, 모델이 없는 것을 있는 것처럼 "보는" 것과 같다.
...
VLM은 언어와 시각 정보를 통합해야 하므로 추가적인 도전이 있다:
...
잘 설계된 데이터 파이프라인은 **자기 강화 루프**를 만든다:
...
데이터 설계는 단순히 데이터를 많이 모으는 문제가 아니다.
**무엇을 가르칠 것인지(태스크), 어떻게 가르칠 것인지(커리큘럼), 얼마나 정확하게 가르칠 것인지(검증)** 를 모두 고려하는 공학적 과정이다.
