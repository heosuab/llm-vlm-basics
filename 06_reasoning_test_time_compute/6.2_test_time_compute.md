...
> **학습 목표**: 모델을 재학습하지 않고도 **추론 시간(inference time)에 더 많은 계산**을 투입하여 성능을 높이는 다양한 기법들. "Thinking harder, not just training harder" 패러다임.
...
Test-time search (Best-of-N, MCTS 등)를 하려면 **어떤 추론 경로가 좋은지 평가하는 기준**이 필요함. 여기서 Reward Model(RM)이 등장함.
...
이처럼 step-level human annotation이 필요하므로 비용이 매우 큼. 이 때문에 **자동화된 PRM 학습** 연구가 활발히 진행되고 있음.
...
**PRM이 일관적으로 ORM을 크게 앞섬.** 특히 N이 클수록 차이가 뚜렷함.
...
수학, 코딩, 논리 퍼즐 등의 태스크에서는 **정답 여부를 자동으로 검증**할 수 있음:
...
DeepSeek-R1에서 사용한 방법. PPO 대신 더 간단한 방식으로 RL을 적용함:
...
이것이 바로 **자기 개선(self-improvement)** 루프이며, 이론적으로 human annotation 없이 무한히 성능 향상이 가능함 (물론 실제 한계는 있음).
...
일반적인 CoT fine-tuning에서 사용하는 reasoning chain은 보통 수십~수백 토큰. 매우 어려운 문제 (IMO 수준 수학, 복잡한 코딩)에서는 이 정도 추론으로는 부족함.

### Long CoT의 핵심 특성

**Long CoT**는 수천 토큰에 달하는 매우 긴 reasoning chain을 통해 모델이 다음을 학습하도록 한다:

1. **탐색(Exploration)**: 여러 접근법을 시도해봄
2. **자기 검증(Self-verification)**: 중간 결과를 스스로 확인함
3. **역추적(Backtracking)**: 막히면 되돌아가 다른 방법을 시도함
4. **자기 수정(Self-correction)**: 오류를 발견하면 즉시 수정함
...
DeepSeek-R1 (2025)은 Long CoT training의 대표적 성공 사례:
...
**핵심 발견**: 명시적으로 backtracking이나 self-verification을 가르치지 않았지만, **RLVR만으로 모델이 스스로 이런 행동을 발견**했음. "Aha moment" 현상이라고도 부름.

### Long CoT의 "Thinking Tokens" 개념

o1, DeepSeek-R1 등에서는 **생성하지만 사용자에게는 보여주지 않는** 특수한 thinking 토큰 구간을 둠:
...
내부 사고 과정에서는 모델이 매우 자유롭게 탐색할 수 있고, 최종 출력에서는 정제된 답변만 제공함.
...
**Best-of-N (BoN)**은 직관적임:
1. N개의 서로 다른 completion을 샘플링함
2. Reward model 또는 verifier로 가장 좋은 것을 선택함
...
하지만 이건 이상적인 경우. 실제로는 **완벽한 verifier가 없으므로** RM score가 높은 샘플이 항상 정답인 것은 아님.
...
OpenAI의 o1 (2024.09)과 DeepSeek-R1 (2025.01)의 핵심 혁신은 **추론 토큰을 test-time에 충분히 생성하도록 학습**한 것. 더 어려운 문제일수록 더 많은 "생각" 토큰을 사용함.
...
흥미롭게도, 단순히 thinking token을 강제로 늘리는 것보다는 **모델이 스스로 필요한 만큼 생각하도록** 학습하는 것이 중요함.
...
이상적인 시스템은 **문제 난이도를 감지하고 compute를 동적으로 배분**함:
